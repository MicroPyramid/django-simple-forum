...
from .forms import CategoryForm, BadgeForm, RegisterForm, TopicForm, CommentForm, UserProfileForm, ChangePasswordForm
try:
...
class CategoryList(AdminMixin, ListView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if request.POST.get('is_active') == 'True':
            category_list = category_list.filter(status=True)
...
class CommentDelete(AdminMixin, DeleteView):
    model = Comment
...
def facebook_login(request):
    if 'code' in request.GET:
        ...
        if 'email' in profile.keys():
            if User.objects.get_or_create(username=profile['email'], email=profile['email']):
